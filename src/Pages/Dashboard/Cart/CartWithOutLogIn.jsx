...
const CartWithOutLogIn = () => {
  ...
  const handleOrder = async () => {
    const { value: formValues } = await Swal.fire({
      html: `
         <div style="max-width: 700px; height: 700px;" >

                <p style="font-size: 1.25rem; font-weight: 600; display: flex; justify-content: flex-start;">Full Name</p>
                ...
                <p style="margin-top: 20px; font-size: 1.25rem; font-weight: 600; display: flex; justify-content: flex-start;">Phone No</p>
        
                <input type="tel" id="mobile" placeholder="+92--" value="+92"  style="  width: 100%;
                padding: 12px;
                border: 1px solid #ccc;
                border-radius: 4px;
                ...
                <p style="font-size: 1.25rem; font-weight: 600; display: flex; justify-content: flex-start;
                  margin-top: 20px; ">Delivery Address</p>
                <input type="text" id="address" placeholder="District, Thana, Place Name, Road No, House Name" style="width: 100%;
                padding: 12px;
                border: 1px solid #ccc;
                border-radius: 4px;
...
`,
    ...
    if (mobileNumber.length == 14 || mobileNumber.length == 11) {
      const orderDetails = {
        email: user?.email || "WithOut LogIn Order",
        name: name,
      ...
      Swal.fire({
        position: "center",
        icon: "warning",
        title: "Please provide a valid mobile number including +880",
        showConfirmButton: false,
        timer: 2000,
      });
    }
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        let outherItems = itemLists.filter(
          (i) => i._id !== existingItem.ItemId
        );

        let outherItems2 = local.filter(
