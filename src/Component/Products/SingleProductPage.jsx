  ...
  const handleAddToCart = () => {
    ...
    if (isAdmin) {
      return Swal.fire({
        position: "top-end",
        icon: "warning",
        title: "Admin will not be able to buy anything.",
    ...
    if (user && user.email) {
      ...
      axiosSecure.post("/carts", addToCart).then((res) => {
      ...
      <div className="bg-white dark:text-black flex relative items-center overflow-hidden">
          ...
          <div className="sm:w-2/3 lg:w-2/5 flex flex-col  relative text-center sm:text-left">
            ...
            <div>
              <p className="flex">
                ...
                <span>
                  <Link
                    to={`/shop/${item?.categories[0]}`}
                    className="btn-link no-underline underline-offset-8"
                  >
                    {" "}
                    {item?.categories[0]}
                  </Link>
        ...
        <span>Add To Cart</span>
                ...
                <button
                  ...
                  className="bg-teal-500 btn text-white hover:scale-105 hover:bg-teal-600 transition duration-300 outline-offset-8 border-none"
                >
                  <span>
                    <CiShoppingCart className=" md:text-2xl " />
                  </span>
                  <span>Add To Cart</span>
                </button>
                ...
                <div className="col-span-2 lg:col-span-1 ">
                  <button
                    ...
                    className="bg-teal-600 btn  border-none text-white hover:scale-105 hover:bg-teal-500 transition duration-300 "
                  >
                    Buy Now
                  </button>
                </div>
          ...
          <div className="sm:w-1/3 lg:w-3/5 flex justify-center sm:ml-20">
            <img
              loading="lazy"
              src={item?.img}
              className="max-w-xs md:max-w-sm m-auto transition-transform transform hover:scale-105"
              alt={item?.name}
            />
          </div>
        </div>
